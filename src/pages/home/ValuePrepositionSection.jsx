...
import expertImg from '../../assets/expertImg1.png'
import careerImg from '../../assets/careerImg1.png'
import newsImg from '../../assets/newsImg1.png'

const cards = [
  {
    title: "Talk To Experts",
    description: "Have an idea? Get advice from professionals anytime.",
    image: expertImg,
    link: "/expert",
  ...
  },
  {
    ...
    description: "Explore career opportunities and guidance to shape your future.",
    image: careerImg,
    link: "/careers",
  ...
  {
    title: "Newsroom",
    description: "Stay updated with the latest news and announcements from our company.",
    image: newsImg,
    link: "/news-room",
  ...
  return (
    <div className="max-w-7xl mx-auto py-30 px-4">
      ...
      <div className="flex justify-center space-x-8">
        {cards.map(({ title, description, image, link, buttonText }) => (
          <div key={title} className="relative text-start w-80 h-80 group cursor-pointer rounded-lg overflow-hidden shadow-lg">
            <img
              src={image}
              alt={title}
            ...
            />
            {/* Overlay */}
            <div className="absolute inset-0 bg-black/50 bg-opacity-50 opacity-0 group-hover:opacity-70 transition-opacity duration-300"></div>
            {/* Content that slides up from bottom */}
            <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black via-transparent to-transparent translate-y-full group-hover:translate-y-0 transition-transform duration-500 flex flex-col items-start text-start text-white">
